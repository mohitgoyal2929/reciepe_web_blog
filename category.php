<?php 
  ...
  if(!isset($_GET['id'])) {
    die('Invalid Category Page request. Please mention category id');
  }

  $category_id = $_GET['id'];

  $query = 'SELECT * FROM `recipe_category` WHERE `category_id` = '.$_GET['id'];
  ...
  if(!empty($post_ids)) {
    $query = 'SELECT * FROM recipe WHERE id IN ('. implode(',', $post_ids).')';
    $stmt = $db->prepare($query);
    $stmt->execute();
